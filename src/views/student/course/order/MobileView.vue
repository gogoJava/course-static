<template>
  <div>
      ...
      <div class="mobile-class-attendance-page">
        <div class="clearfix" v-if="tableData.list.length">
          <div>
            ...
            <span style="padding-left: 10px;color: #409EFF;font-size: 12px;" v-if="!rosterId && !loading && bought" @click="confirmSeat">确定座位</span>
            <span style="padding-left: 10px;color: #409EFF;font-size: 12px;" v-if="!bought && !orderStatus" @click="buyCourseShow = true">购买课程</span>
          </div>
          <div v-if="orderStatus">
            <div style="font-size: 14px;font-weight: bold;padding-top: 15px;">
              ...
              <span v-if="orderStatus && orderStatus === '0' && orderId" class="item">
                <span>微信支付：</span>
                <icon-font icon="wxpay" class="icon" size="22px" style="color: #67C23A;cursor: pointer;" @click.native="goPay()"></icon-font>
              </span>
        ...
        <div v-if="tableData.list.length" style="padding-top: 30px;overflow-x: auto;">
    ...
    <el-dialog :visible.sync="buyCourseShow" title="提示" :show-close="false" width="80%" top="120px" class="buy-show">
      ...
      <span slot="footer">
        <div style="text-align: center;">
          <el-button @click="buyCourseShow = false" size="small">取消</el-button>
        <el-button @click="goPayCourse" size="small" type="primary">确定</el-button>
        </div>
      </span>
    </el-dialog>
    <el-dialog :visible.sync="dialogVisible" :show-close="false" custom-class="qrcode-img">
      ...
      <div v-if="payUrl" style="text-align: center;padding-top: 15px;">长按二维码识别支付</div>
    ...
    <el-dialog title="课程详情" :visible.sync="centerDialogVisible" width="80%" center>
      ...
      <span slot="footer" class="dialog-footer">
          <el-button @click="centerDialogVisible = false" size="mini">关闭</el-button>
        </span>
    </el-dialog>
  </div>
</template>
...
<style>
  ...
  .mobile-class-attendance-page .seat-icon .icon-selected {
    /*color: #409EFF;*/
  }
  ...
  .mobile-class-attendance-page .el-checkbox-group .el-checkbox.is-bordered.el-checkbox--mini {
    padding: 3px 0;
    height: 24px;
  }
  .mobile-class-attendance-page .el-checkbox-group .el-checkbox.is-bordered+.el-checkbox.is-bordered {
    margin-left: 2px;
  }
  .mobile-class-attendance-page .chenk-box {
    width: 50px;
